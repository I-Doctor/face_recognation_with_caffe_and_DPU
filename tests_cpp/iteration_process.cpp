...
// File Name     : iteration_process.cpp
// Description   : read pairs of picture name and process them with dpu
//                 one by one then get the result of face recognation 
//                 and write into the result file or save in picture.
...
#define TIMING(log_str)do { \
    ...
    printf(log_str,timeuse /1000000.0); \
} while(0)
...
int main(int argc, char* argv[]){
    ...
    for(int pair=0; pair<img_path_pairs.size(); pair++){
        ...
        TIMING("        ##Calculate [result] time: %f\n");

        // save result in image and show -----------------------------------------
        if (save_flag) {
            printf("      --@@Save the image\n");
            save_img(img_1_t._mat,img_2_t._mat, result, "output/"+img_name_1_t+img_name_2_t+".jpg", show_flag);

            TIMING("        ##Save [image] time %f\n"); 
        }
    }

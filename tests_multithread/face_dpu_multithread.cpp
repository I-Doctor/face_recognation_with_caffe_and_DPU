...
void* preprocessing_pth(void* arg);
void* dpu_calculate_pth(void* arg);
void* result_output_pth(void* arg);
...
int main(int argc, char* argv[]){
    ...
    // start init-----------------------------------------------------------
    // variable init
    sem_init(&output_done,0,10);
    sem_init(&process_done,0,0);
    sem_init(&dpu_done,0,0);
    sem_init(&rec_access,0,1);
    ...
    // preprocessing-------------------------------------------------------
    pthread_t* p_p = new pthread_t;
    pthread_create(p_p,NULL,preprocessing_pth,NULL);
    // dpu calculating-----------------------------------------------------
    pthread_t* d_p = new pthread_t;
    pthread_create(d_p,NULL,dpu_calculate_pth,NULL);
    // result & output-----------------------------------------------------
    pthread_t* r_p = new pthread_t;
    pthread_create(r_p,NULL,result_output_pth,NULL);
...
void* preprocessing_pth(void* arg)
{
    ...
    for(int pair=0; pair<N; pair++){
        ...
        if (!input_bin_1 || !input_bin_2) {
            cerr << "failed to create input data file" << endl;
            FATAL;
        }
...
void* dpu_calculate_pth(void* arg)
{
    ...
    for(int pair=0; pair<N; pair++){
        ...
        sem_post(&dpu_done);
        //int temp;
        //sem_getvalue(&dpu_done, &temp);
        //printf(" dpu_done:%d", temp);

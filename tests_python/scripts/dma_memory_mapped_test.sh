...
if [ $h2cChannels -gt 0 ]; then
  ...
  do
    ...
    ./dma_to_device -d /dev/xdma0_h2c_${curChannel} -f data/datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount -v &
...
echo "Time: write time."
getTiming $write_begin_time $write_end_time

# Read from all enabled c2hChannels in parallel
read_begin_time=`date +%s.%N`
if [ $c2hChannels -gt 0 ]; then
  ...
  do
    ...
    ./dma_from_device -d /dev/xdma0_c2h_${curChannel} -f data/output_datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount -v &
    # If all channels have active transactions we must wait for them to complete
...
echo "Time: read time."
getTiming $read_begin_time $read_end_time

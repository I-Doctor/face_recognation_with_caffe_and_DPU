...
static int test_dma(char *devicename, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filename)
{
  ...
  while (count--) {
    ...
    /* user debug: written by zhongkai @2018.10.26 */
    printf("RD lseek fpga_fd: addr successfully\n");
    rc = clock_gettime(CLOCK_MONOTONIC, &ts_start);
    /* read data from AXI MM into buffer using SGDMA */
    rc = read(fpga_fd, buffer, size);
    /* user debug: wirtten by zhongkai @2018.10.26 */
    printf("RD read from fpga_fd to buffer finish\n");
    if ((rc > 0) && (rc < size)) {
    ...
    if ((file_fd >= 0) & (no_write == 0)) {
      ...
      printf("RD write buffer to file\n");
